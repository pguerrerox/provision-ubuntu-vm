...
validate_cidr() {
  local ipcidr="$1"
  [[ "$ipcidr" =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}/([0-9]|[1-2][0-9]|3[0-2])$ ]]
}
...
choose_interface() {
  ...
  while true; do
    read -r -p "Select interface number to configure: " idx
...
configure_static_ip() {
  ...
  while true; do
    read -r -p "Enter new static IP in CIDR format (example 192.168.1.50/24): " ipcidr
  ...
  if ask_yes_no "Override gateway?" "n"; then
    while true; do
      read -r -p "Enter gateway IP: " gateway
    ...
    if [[ -z "$gateway" ]]; then
      while true; do
        read -r -p "No gateway detected. Enter gateway IP: " gateway
        if validate_ip "$gateway"; then
          break
        fi
        echo "Invalid IP."
  ...
  if ask_yes_no "Override DNS servers?" "n"; then
    read -r -p "Enter DNS servers separated by spaces (example: 1.1.1.1 8.8.8.8): " dns_input
    dns_list="$dns_input"
  fi
